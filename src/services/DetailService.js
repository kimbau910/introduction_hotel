...
export const getAlldetail = async (search, limit) => {
    let res = {}
    if (search?.length > 0) {
        res = await axios.get(`http://localhost:3001/api/detail/get-all?filter=name&filter=${search}&limit=${limit}`)
    } else {
        res = await axios.get(`http://localhost:3001/api/detail/get-all?limit=${limit}`)
    }
    return res.data
}
...
export const getDetailType = async (type, page, limit) => {
    if (type) {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/detail/get-all?filter=type&filter=${type}&limit=${limit}&page=${page}`)
        return res.data
    }
}
...
export const getDetailsDetail = async (id) => {
    const res = await axios.get(`${process.env.REACT_APP_API_URL}/detail/get-details/${id}`)
    return res.data
}
